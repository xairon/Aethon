...
def _create_tts(config: JarvisConfig):
    ...
    if config.tts.backend == "chatterbox":
        from jarvis.tts.chatterbox import ChatterboxSynthesizer
...
def _create_wake_detector(config: JarvisConfig):
    """Factory : crée le détecteur de wake word selon le mode configuré."""
    if config.persona.wake_mode == "whisper":
        from jarvis.wakeword.whisper_detector import WhisperWakeDetector
...
class JarvisPipeline:
    ...
    def __init__(self, config: JarvisConfig):
        ...
        self._api_server = None     # JarvisAPIServer (Phase 3)
    ...
    def _start_api_server(self):
        ...
        self._api_server = JarvisAPIServer(self, port=port)
    ...
    def _load_tools(self):
        ...
        if self.config.tools.enable_system_info:
            from jarvis.tools.system_tool import SystemInfoTool
