...
class JarvisAPIServer:
    """Serveur API REST pour controler Jarvis a distance.

    Endpoints :
        POST /api/command  {"text": "..."} → reponse texte JSON
        POST /api/speak    {"text": "..."} → reponse texte + audio WAV
        POST /api/wake     → active Jarvis (simule le wake word)
        GET  /api/status   → etat du pipeline
        GET  /api/tools    → liste les outils disponibles
    """
    ...
    def _run(self):
        ...
        try:
            ...
            log.info("API Jarvis démarrée sur http://127.0.0.1:%d", self._port)
    ...
    async def _handle_speak(self, request: web.Request) -> web.Response:
        """POST /api/speak {"text": "bonjour jarvis"}
    ...
    def _execute_speak(self, text: str) -> tuple[str, bytes] | None:
        ...
        try:
            from jarvis.tts.emotion import parse_emotion_tags, strip_emotion_tags
            from jarvis.tts.text_prep import prepare_for_tts
    ...
    async def _handle_wake(self, request: web.Request) -> web.Response:
        """POST /api/wake — Active Jarvis (simule un wake word).

        Si le wake word est désactivé, Jarvis est déjà actif en permanence.
        """
        if not self._pipeline.config.persona.wake_enabled:
            return web.json_response({
                "status": "already_active",
                "message": "Wake word désactivé, Jarvis écoute en permanence.",
            })
        ...
        if was_active:
            log.info("API wake: Jarvis était déjà actif.")
            return web.json_response({"status": "already_active"})

        log.info("API wake: Jarvis activé à distance.")
