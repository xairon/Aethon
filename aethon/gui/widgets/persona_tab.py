...
WAKE_MODELS = ["hey_aethon", "alexa", "hey_mycroft", "ok_google"]
...
class PersonaTab(QWidget):
    ...
    def __init__(self, config: AethonConfig, parent=None):
        ...
        self._whisper_phrase.setPlaceholderText("Ex: Salut Aethon, Bonjour assistant...")
    ...
    def _import_wav(self):
        ...
        from aethon.gui.widgets.voice_import_dialog import VoiceImportDialog
    ...
    def _open_hf_browser(self):
        ...
        from aethon.gui.widgets.voice_browser import VoiceBrowserDialog
    ...
    def apply(self, config: AethonConfig):
        # Identité
        config.persona.name = self._name.text().strip() or "Aethon"
        ...
        if config.persona.wake_mode == "whisper":
            config.persona.wake_phrase = (
                self._whisper_phrase.text().strip() or "salut aethon"
            )
        else:
